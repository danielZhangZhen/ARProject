...
public class IsDownloadInCarController : MonoBehaviour {

	public string nextSceneName = "";//下一个场景名称
	public string downloadAssetBundleName="";//从服务器上下载的AB包名称,本app未使用到此变量
	#if UNITY_ANDROID
	private string downloadURL = "http://lmo.oss-cn-shenzhen.aliyuncs.com/2B/DW_ShunXiOuGe/AssetBundles/Android/sxog-incar-android.unity3d";//服务器下载地址
	#else
	private string downloadURL = "http://lmo.oss-cn-shenzhen.aliyuncs.com/2B/DW_ShunXiOuGe/AssetBundles/Android/sxog-incar-ios.unity3d";
	#endif
	public GameObject tipsUI;
	public GameObject loadingUI;
	public Image loadingPercent;
	private int version = 15; //版本号控制
	...
	public void LoadScene(string loadSceneName){
		...
		if(PlayerPrefs.GetInt("IsDownloadInCar")!=version){
...
//			loadingUI.SetActive (true);
//			StartCoroutine (DownloadSceneAssetBundle ());
//		} else if(string.Compare(clickArrow,"SXOG_10") ==0){ //为了防止两个箭头上的脚本同时下载
//			tipsUI.SetActive (false);
//			loadingUI.SetActive (true);
//			StartCoroutine (DownloadSceneAssetBundle ());
//		}
	...
	IEnumerator DownloadSceneAssetBundle(){		
		...
		using (WWW www = WWW.LoadFromCacheOrDownload (downloadURL, 11)) {
			m_www = www;
...
//				cache.Close(); 
				PlayerPrefs.SetInt("IsDownloadInCar",version);
				BackController.Instance.bundleInCar = www.assetBundle;
				V1_Loading.LocalSceneName = nextSceneName;
				SceneManager.LoadScene ("V1_Loding_wide");
			}
		}
